...
export const getDashboardStats = async (_req, res) => {
  try {
    ...
    const [
      ...
      withdrawalAgg,
      b2bPending, subsidyPending,
      monthlyRevenueAgg,
    ] = await Promise.all([
      Order.aggregate([{ $match: { paymentStatus: 'paid' } },                  { $group: { _id: null, total: { $sum: '$total' } } }]),
      Order.aggregate([{ $match: { paymentStatus: 'paid', createdAt: { $gte: startThisMonth } } }, { $group: { _id: null, total: { $sum: '$total' } } }]),
      Order.aggregate([{ $match: { paymentStatus: 'paid', createdAt: { $gte: startLastMonth, $lt: startThisMonth } } }, { $group: { _id: null, total: { $sum: '$total' } } }]),
      ...
      Withdrawal.aggregate([
        { $facet: {
          pending:   [{ $match: { status: 'pending' } },   { $group: { _id: null, count: { $sum: 1 }, amount: { $sum: '$amount' } } }],
          completed: [{ $match: { status: 'completed' } }, { $group: { _id: null, total: { $sum: '$amount' } } }],
        }},
      ]),
      ...
      Order.aggregate([
        { $match: { paymentStatus: 'paid', createdAt: { $gte: start6MonthsAgo } } },
        { $group: {
    ...
    const wdPending   = withdrawalAgg[0]?.pending?.[0]   || { count: 0, amount: 0 }
    const wdCompleted = withdrawalAgg[0]?.completed?.[0] || { total: 0 }
    ...
    const revGrowth  = lastRev > 0 ? +((thisRev - lastRev) / lastRev * 100).toFixed(1) : 0

    res.json({
      success: true,
      stats: {
        revenue: { total: totalRev, thisMonth: thisRev, lastMonth: lastRev, growth: revGrowth },
        orders:  {
        ...
        withdrawals: { pending: wdPending.count, pendingAmount: wdPending.amount, totalPaid: wdCompleted.total },
        b2bPending,
        subsidyPending,
        monthlyRevenue,
      },
