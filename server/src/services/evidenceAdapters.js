...
export const fulfilmentAdapter = { async collect(businessId) { const business = await Business.findById(businessId).lean(); return { state: business?.state?.fulfilment === 'ACTIVE' ? 'VALID' : 'UNCONFIGURED', configured: business?.state?.fulfilment === 'ACTIVE' } } }
