...
export default function ActivityPage() {
  ...
  return <main className="section-wrapper max-w-4xl py-10"><PageMeta title="Activity" noIndex /><p className="eyebrow">{selectedBusiness.name}</p><h1 className="page-title mt-2">Business activity</h1>{error && <p className="mt-4 text-sm text-red-700">{error}</p>}{!events.length ? <section className="surface-card mt-7 p-6"><h2 className="font-bold">No activity yet</h2><p className="mt-2 text-sm text-slate-600">Business updates will appear here as you build, source, and operate.</p></section> : <section className="surface-card mt-7 divide-y divide-slate-100">{events.map(event => <div key={event._id} className="p-5"><p className="font-semibold text-slate-900">{event.eventType.replaceAll('_', ' ')}</p><p className="mt-1 text-sm text-slate-500">{new Date(event.createdAt).toLocaleString()}</p></div>)}</section>}</main>
}
