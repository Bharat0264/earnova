    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1 lg:ml-56 xl:ml-60 min-w-0">
        ...
        <main className="p-5 lg:p-8 space-y-6">
          ...
          {tab === 'ca'          && <><div className="rounded-2xl border border-primary-200 bg-primary-50 p-5"><p className="font-display font-bold text-primary-950">Firm-based CA office administration</p><p className="mt-1 text-sm text-primary-800">Manage verified firms, team designations, configurable services, new cases and isolation-aware assignments.</p><Link to="/admin/ca/firms" className="btn-primary mt-4 text-sm">Open CA office admin</Link></div><CAWorkTable profiles={caProfiles.data} taxJobs={caTaxJobs.data} caProfiles={caTaxJobs.caProfiles} loadingProfiles={caProfiles.loading} loadingJobs={caTaxJobs.loading} reloadProfiles={() => { caProfiles.reload(); caTaxJobs.reload(); reloadStats() }} reloadJobs={() => { caTaxJobs.reload(); reloadStats() }} /></>}
