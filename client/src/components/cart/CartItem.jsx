...
export default function CartItem({ item }) {
  ...
  const { _id, name, slug, brand, price, gstRate = 18, quantity, images, thumbnail, category } = item
    ...
    <div className="flex gap-4 py-5 border-b border-gray-100 last:border-0">
      ...
      <Link
        ...
        className="w-20 h-20 sm:w-24 sm:h-24 rounded-xl overflow-hidden shrink-0
                   bg-gray-50 border border-gray-100 hover:opacity-90 transition-opacity"
      >
        {(thumbnail || images?.[0]) ? (
          <img
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-gray-400 mt-1">
          Incl. {gstRate}% GST ({formatPrice(gst)})
        </p>
